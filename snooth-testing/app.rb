...
# price_start = 0; # 0 cents
price_increment = 0.50; # 1 cent
price_start = 0.50; # 49 cents
...
# 	else 
# 		# puts "Received #{object['meta']['results']} wines. Price increment from $#{(i / 100.0)} to #{(i / 100.0)}"
...
for i in 0..1200
    request = {'q' => 'wine', 't' => 'wine', 'a' => 0, 'n' => MAX_N, 'mp' => i*price_increment+price_start, 'xp' => i*price_increment+price_start}
	object = Snooth_Handler.executeSnoothRequest(request)
	if object['wines'].nil?
		puts "Error: received 0 wines. Price increment from $#{i*price_increment} to #{(i+1)*price_increment}"
	else 
	...
	if object['meta']['results'] > MAX_N
		mini_requests = ((object['meta']['results'] -1) / MAX_N).ceil
			for j in 1..mini_requests
				request = {'q' => 'wine', 't' => 'wine', 'a' => 0, 'n' => MAX_N, 'mp' => i*price_increment+price_start, 'xp' => i*price_increment+price_start, 'f' => (j*MAX_N + 1)} 
				object = Snooth_Handler.executeSnoothRequest(request)
				if object['wines'].nil?
					puts "Double Error: received 0 wines. Price increment from $'#{i*price_increment+price_start} to '#{(i+1)*price_increment+price_start}"
				else	
					object['wines'].each do |wine_json|
						puts "'#{wine_json['name']}','#{wine_json['code']}','#{wine_json['region']}','#{wine_json['winery']}','#{wine_json['winery_id']}','#{wine_json['varietal']}','#{wine_json['price']}','#{
 							wine_json['vintage']}','#{wine_json['type']}','#{wine_json['link']}','#{wine_json['tags']}','#{wine_json['image']}','#{wine_json['snoothrank']}','#{wine_json['available']}','#{wine_json['num_merchants']}','#{
